...
status_t pop_end(list_t *p_list, data_t *p_end_data)
{
	...
	get_last_node_and_prev(p_list, &p_last_node, &p_last_prev);
	*p_end_data = p_last_node->data;
	generic_delete(p_last_prev, p_last_node);
	return SUCCESS;
}
...
status_t remove_data(list_t *p_list, data_t r_data)
{
	...
	get_node_and_prev(p_list, r_data, &p_data_node, &p_prev_node);
	if(p_data_node == NULL)
		return LIST_DATA_NOT_FOUND;
	generic_delete(p_prev_node, p_data_node);
	return SUCCESS;
}
...
/* definations of auxillary routines */
